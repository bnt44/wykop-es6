...
class Wykop {
	...
	*/
	constructor(appkey, secretkey, {output, format, timeout=30000, useragent='WypokAgent', accountkey, userkey, autologin=false, retryCount=1}={}) {
		assert(appkey && secretkey, 'appkey and secretkey cannot be null');
		_.assign(this, {appkey, secretkey, output, format, timeout, useragent, accountkey, userkey, autologin, retryCount});
		...
		return new _Promise((resolve, reject) => {
			doRequest(options, (error, response, body) => {
				if (error) {
					...
					if ((code === 11 || code === 12 || code === 13) && this.autologin && this[errLoginCount] < this.retryCount) {
						this._errLoginCount++;
